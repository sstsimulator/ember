...
int main(int argc, char* argv[]) {
	...
	if( me < 2 ) {
		...
		if( 0 == me ) {
			...
			const double timeTaken  = (((double) end.tv_sec) + 1.0e-6 * ((double) end.tv_usec)) -
			...
			printf("# %9s %11s %17s %14s %16s %14s\n",
				"MsgSize", "Time", "KMsgs", "MB",
				"KMsg/S", "MB/S");
			printf("  %9.0f %11.4f %17.5f %14.4f %16.4f %14.4f\n",
				(double) msgSize, timeTaken, KMsgsXchng, MbytesXchng,
				KMsgsXchng / timeTaken, MbytesXchng / timeTaken);
		}
	}
